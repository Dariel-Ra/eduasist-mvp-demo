...
class GuardianObserver
{
    ...
    public function updated(Guardian $guardian): void
    {
        if ($guardian->wasChanged('phone_number')) {
            \Log::info("Teléfono actualizado para tutor ID {$guardian->id}: {$guardian->phone_number}");
        }

        if ($guardian->wasChanged('whatsapp_number')) {
            \Log::info("WhatsApp actualizado para tutor ID {$guardian->id}: {$guardian->whatsapp_number}");
        }
    }
